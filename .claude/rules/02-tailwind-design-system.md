...
> Tailwind v4 design system — tokens, @utility classes, pure utility styling
...
| Utility | Use for |
|---------|---------|
| `icon-btn-subtle` | Light/dark chrome social buttons (contact page, light surfaces) |
| `footer-shell` / `footer-heading` / `footer-nav-link` / `footer-contact-link` / `footer-contact-static` / `footer-social-btn` / `footer-pill-link` / `footer-cta-band` / `footer-legal-link` | Always-dark site footer chrome (`bg-slate-950`) |
