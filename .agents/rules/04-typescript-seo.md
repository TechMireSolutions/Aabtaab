...
- JSON-LD: use helpers under `lib/seo/json-ld*.tsx` (import via `@/lib/seo`) — do not hand-roll schema.
...
- Media must stay within the viewport (`max-w-full` / `next/image` + aspect utilities) — see rule `13`.
