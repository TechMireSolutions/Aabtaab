    ...
    <section className={`py-10 md:py-16 border-b border-gray-100 ${bgClass}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-7 sm:mb-10">
          ...
          <div className="flex items-center gap-3 shrink-0 sm:ml-6">
            {/* Prev / Next */}
        ...
        <div className="relative">
          ...
          <div
            className={`absolute left-0 top-0 bottom-0 w-10 z-10 pointer-events-none
              bg-linear-to-r ${bg === "gray" ? "from-slate-50" : "from-white"} to-transparent
              transition-opacity duration-200 ${canLeft ? "opacity-100" : "opacity-0"}`}
          />
          {/* Right fade */}
          <div
            className={`absolute right-0 top-0 bottom-0 w-10 z-10 pointer-events-none
              bg-linear-to-l ${bg === "gray" ? "from-slate-50" : "from-white"} to-transparent
          ...
          >
            {items.map((item, i) => (
              <div
                key={item.id}
                data-card
                className="shrink-0 w-[272px] sm:w-[296px] lg:w-[316px]"
