...
export const event = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      rows:  3,
      group: 'details',
      description: 'Used in event cards and as the JSON-LD description. Keep under 200 characters.',
    }),
    defineField({
      name:  'body',
      type:  'array',
  ...
  orderings: [
    { title: 'Start Date (soonest first)', name: 'startDateAsc',  by: [{ field: 'startDate', direction: 'asc' }] },
    { title: 'Start Date (latest first)',  name: 'startDateDesc', by: [{ field: 'startDate', direction: 'desc' }] },
  ],
  preview: {
    ...
    prepare({ title, date, media, city }) {
      ...
      return { title, subtitle: `${formatted}${city ? ` · ${city}` : ''}`, media }
    },
  },
})
